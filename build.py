...
REGIONS = ["서울", "인천", "경기북부", "경기남부", "강원", "충청", "전라", "경상", "제주"]
REGION_ICON = {"서울": "🏙️", "인천": "⚓", "경기북부": "🌲", "경기남부": "🏡",
               "강원": "⛰️", "충청": "🌾", "전라": "🌽", "경상": "🌊", "제주": "🌴"}

# 경기 북부(경기도 북부청 관할 10시군) — 나머지 경기는 남부
GG_NORTH = {"고양", "구리", "남양주", "동두천", "양주", "의정부", "파주", "포천", "연천", "가평"}


def display_region(p):
    """수도권을 area로 서울·인천·경기북부·경기남부로 세분. 그 외 권역은 그대로."""
    r = p.get("region", "")
    if r != "수도권":
        return r
    a = p.get("area", "") or ""
    ...
    if a.startswith("경기"):
        toks = a.split()
        city = re.sub(r"(특별자치시|특별시|광역시|시|군|구)$", "", toks[1]) if len(toks) > 1 else ""
        return "경기북부" if city in GG_NORTH else "경기남부"
    return "수도권"
...
def build():
    ...
    for cat in CATEGORIES:
        ...
        for region in REGIONS:
            in_region = [p for p in in_cat if display_region(p) == region]
            if not in_region:
                continue
...
<style>
  ...
  * { -webkit-tap-highlight-color: transparent; }
  ...
  .resultbar { display: flex; justify-content: flex-end; align-items: center; flex-wrap: wrap; gap: 6px;
               border-bottom: 1px solid #d4d4d9; padding-bottom: 14px; margin-bottom: 6px; }
